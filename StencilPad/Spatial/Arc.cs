...
public struct Arc
{
    ...
    public Unit2D Start
    {
        get
        {
            return new Unit2D(_center.X + _radius * Math.Sin(_startAngle),
                              _center.Y + _radius * Math.Cos(_startAngle));
        }
    }
    ...
    public Unit2D End
    {
        get
        {
            return new Unit2D(_center.X + _radius * Math.Sin(_endAngle),
                              _center.Y + _radius * Math.Cos(_endAngle));
        }
    }
    ...
    public Arc(Unit2D start, Unit2D mid, Unit2D end)
    {
        ...
        _startAngle = Math.Atan2((start.X - _center.X).Millimeters,
                                 (start.Y - _center.Y).Millimeters);

        _endAngle = Math.Atan2((end.X - _center.X).Millimeters,
                               (end.Y - _center.Y).Millimeters);
    }
